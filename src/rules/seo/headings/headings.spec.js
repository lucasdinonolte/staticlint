...
describe('seo.headings.hasH1', () => {
  it('should error if there is no H1 tag on the page', async () => {
    ...
    expect(results.all[0]).toBe('There should be 1 H1 tag. Found 0.')
  })
  ...
  it('should error if there is more than one H1 tag on the page', async () => {
    ...
    expect(results.all.length).toBe(1)
    expect(results.all[0]).toBe('There should be 1 H1 tag. Found 2.')
  })
})
...
describe('seo.headings.notEmpty', () => {
  it('should error if heading tags are empty', async () => {
    ...
    expect(results.all[0]).toBe('Headings should not be empty')
  })
})

describe('seo.headings.idealLength', () => {
  it('should error if heading tags are too long', async () => {
    let results = await runTestForRule(
      headingsIdealLength,
      '<h1>This heading is super long, it really should not be that long. That’s a few characters. Really hard to read. Such text. Wow. Amaze.</h1>',
    )
    expect(results.all.length).toBe(1)
  })
...
describe('seo.headings.levels', () => {
  it('should warn if a heading level is skipped', async () => {
    ...
    expect(results.all.length).toBe(1)
    expect(results.all[0]).toBe('There are h3 tags but no h2 tag')
  })
})
